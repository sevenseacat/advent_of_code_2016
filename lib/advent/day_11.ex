defmodule Advent.Day11 do
  ...
  def do_search([], next_level_states) do
    IO.puts "* Going to level #{next_level_states |> hd |> elem(0) |> length}: #{length(next_level_states)} states to check in this level."
    do_search(next_level_states, [])
  end
  def do_search([{_path, []} | alt_paths], next_level_states), do: do_search(alt_paths, next_level_states)
    ...
    case State.winning?(state) do
      ...
      false ->
        ...
        case state in path do
          true -> do_search([{path, states} | alt_paths], next_level_states)
          false -> do_search([{path, states} | alt_paths], [{[state | path], State.legal_moves(state)} | next_level_states])
        end
    end
  end
end
