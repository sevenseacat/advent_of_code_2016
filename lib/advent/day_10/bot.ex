defmodule Advent.Day10.Bot do
  use GenServer
  alias Advent.Day10.{Bot, Output}
  ...
  def handle_call(:run, _from, {_number, pieces, _rule, _comps}=state) when length(pieces) < 2 do
    {:reply, :pending, state}
  end
  ...
  def handle_call(:comparisons, _from, {_number, _pieces,  _rule, comps}=state), do: {:reply, comps, state}
